...
func quote(client *whatsmeow.Client, ctx *sve_context.Context) error {
	...
	if err == nil {
		senderName = senderInfo[senderJID].PushName
		if senderName == "" {
			senderName = senderInfo[senderJID].VerifiedName
		}
	}
	if senderName == "" {
	...
	msgTimestamp := time.Unix(int64(contextInfo.GetQuotedMessageTimestamp()), 0)
...
func createQuoteImage(pfp image.Image, name, text string, timestamp time.Time) ([]byte, error) {
	...
	headerHeight := 30.0
	footerHeight := 30.0
	totalHeight := headerHeight + textBlockHeight + footerHeight + (padding * 2)

	dc := gg.NewContext(int(finalWidth), int(totalHeight))
	dc.SetColor(darkBgColor)
	dc.Clear()

	dc.SetColor(darkBubbleColor)
	dc.DrawRoundedRectangle(padding, padding, finalWidth-(padding*2), totalHeight-(padding*2), bubbleRadius)
	dc.Fill()
	...
	dc.SetLineSpacing(lineSpacing)
	dc.DrawStringWrapped(text, contentX, contentY, 0, 0, (finalWidth - contentX - padding), 1, gg.AlignLeft)

	timeStr := timestamp.Format("3:04 PM")
	timeWidth, _ := dc.MeasureString(timeStr)
	timeX := finalWidth - padding - timeWidth - 15.0
	timeY := totalHeight - padding - 15.0
	...
	if err := webp.Encode(&buf, finalImage, &webp.Options{Lossless: true}); err != nil {
